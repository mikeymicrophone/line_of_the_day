ActionController::Routing::Routes.draw do |map|
  ...
  map.resources :approaches, :has_one => [:line, :scene, :result], :collection => {:auto_complete_for_line_phrasing => :post, :auto_complete_for_scene_name => :post, :auto_complete_for_result_name => :post}, :has_many => [:comments]
  ...
  map.resources :lists, :has_many => [:list_items], :member => {:elevate => :put, :lower => :put, :tag => :get, :reorder => :get}, :collection => {:sort => :post}
